...
**The same root cause appears a third time, with a much narrower trigger.**
`shimStylesContent` and `shimContentAttribute`
(`platform-browser/src/dom/dom_renderer.ts:102-112`) both use `compId` as a replacement string:

```js
const shimContentAttribute = (compId) => CONTENT_ATTR.replace(COMPONENT_REGEX, compId);
const shimStylesContent = (compId, styles) => styles.map((s) => s.replace(COMPONENT_REGEX, compId));
```

`compId` is `appId + '-' + component.id` (`:690`), and `appId` is the developer-set `APP_ID` token.
The two calls substitute into _different surrounding text_ — a bare attribute name versus a whole
stylesheet — so a replacement pattern that depends on context makes them disagree:

```
APP_ID      content attribute             selector inside the CSS
ng          _ngcontent-ng-c123456789      _ngcontent-ng-c123456789        match
my$app      _ngcontent-my$app-c123456789  _ngcontent-my$app-c123456789    match
a$'b        _ngcontent-ab-c123456789      _ngcontent-a                    MISMATCH -> styles never apply
a$`b        _ngcontent-a_ngcontent-b-...  _ngcontent-a.title[_ngcontent-  MISMATCH -> styles never apply
```

When they disagree the emitted selector no longer matches the attribute Angular stamps on the
element, so the component's styles silently never apply. Only the two context-dependent forms break
it; `$&` happens to survive because the matched text is `%COMP%` in both places, and `$$` and a lone
`$` are fine.

Recorded here rather than as its own finding because the trigger is far weaker than the two above:
`APP_ID` is a build-time constant a developer chooses, and neither dollar-quote nor
dollar-backtick in one is a realistic mistake. It earns a mention only as the third instance of the pattern — a caller-supplied
string used as a `replace` replacement without escaping.
...
| Can a hostile URL survive the router's parse/serialize round trip changed, or escape the origin?                                                                                                                     | No. Ran the real `DefaultUrlSerializer` both ways: every structural character is lossless across all six URL positions, 18 redirect-shaped inputs are stable on a second pass, and a leading `//` collapses to an ordinary first segment. Details in [26 § 11](./26-security-sinks.md).                                                                                                                                   |
| Does `platform-browser` expose an injection sink?                                                                                                                                                                    | No. Swept `shared_styles_host.ts`, `dom_renderer.ts` and `key_events.ts`: `<style>` content and `<link href>` come from compile-time component metadata; the `appId` interpolated into a `querySelectorAll` selector (`shared_styles_host.ts:72`) and the `namespace` indexing `NAMESPACE_URIS` (`dom_renderer.ts:392`) are both developer-authored, and a bad value throws visibly rather than widening anything.        |
| The HTML sanitizer's allowlists (`VALID_ELEMENTS`, `VALID_ATTRS`) are plain objects built by `tagSet`/`merge` (`sanitization/html_sanitizer.ts:18-32`) — can an inherited name pass them?                            | Not in the sanitizer. All four of its checks use `hasOwnProperty` (`:182`, `:184`, `:193`, `:208`). The one bare index, `URI_ATTRS[lower]` at `:199`, is gated by the `hasOwnProperty` test six lines above, so `lower` is already a real attribute name by then.                                                                                                                                                         |
| `i18n_parse.ts:841` checks the same allowlist with a bare index (`VALID_ATTRS[lowerAttrName]`) while the adjacent binding branch at `:824` uses `hasOwnProperty` — on attribute names taken from translation content | Real inconsistency, no consequence. After the `.toLowerCase()` at `:817`, exactly two inherited names survive — `constructor` and `__proto__` — and neither is a dangerous HTML attribute. Verified that `onerror`, `onclick`, `onload`, `style` and `srcdoc` are rejected by **both** branches. Worth aligning, not worth alarm.                                                                                         |

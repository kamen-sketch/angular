...
await page.goto(`http://localhost:${port}/router-fuzz/impact.html`,{waitUntil:'load'});
await page.waitForFunction('window.__IMPACT__!==undefined',{timeout:600000}).catch(()=>{});
const r=await page.evaluate(()=>window.__IMPACT__??null);
...
await page.screenshot({path:path.join(ROOT,'router-fuzz','f08-impact.png'),fullPage:true});

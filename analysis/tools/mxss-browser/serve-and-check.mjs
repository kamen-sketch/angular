...
await page.goto(`http://localhost:${port}/mxss-browser/real-npm.html`,{waitUntil:'load'});
await page.waitForFunction('window.__REAL__!==undefined',{timeout:20000}).catch(()=>{});
const r=await page.evaluate(()=>window.__REAL__??null);
...
await page.screenshot({path:path.join(ROOT,'mxss-browser','real-npm-proof.png'),fullPage:true});

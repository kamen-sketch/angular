...
await page.goto(`http://localhost:${port}/mxss-browser/fuzz.html`,{waitUntil:'load'});
await page.waitForFunction('window.__FUZZ__!==undefined',{timeout:600000}).catch(()=>{});
const r=await page.evaluate(()=>window.__FUZZ__??null);
...
await page.screenshot({path:path.join(ROOT,'mxss-browser','fuzz-proof.png'),fullPage:true});
console.log('screenshot -> fuzz-proof.png');
